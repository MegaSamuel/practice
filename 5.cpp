...
vector<pair<int, int>> FindTopDocuments(
    ...
    for (auto [document_id, relevance] : FindAllDocuments(word_to_documents, stop_words, query)) {
        found_documents.push_back(make_pair(document_id, relevance));
    }

    // сортируем, используем свой компаратор
    sort(found_documents.begin(), found_documents.end(), 
        [](pair<int, int>& it1, pair<int, int>& it2)
        { 
            // больше тот у кого больше релевантность
            if(it1.second != it2.second) return (it1.second > it2.second);
            
            // больше тот у кого больше документ
            if(it1.first != it2.first) return (it1.first > it2.first);
            
            return false;
        } );

    int size = min(MAX_RESULT_DOCUMENT_COUNT, static_cast<int>(found_documents.size()));

    return {found_documents.begin(), found_documents.begin()+size};
}
